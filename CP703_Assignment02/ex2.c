...
static int parse_sscanf( char * in_str, struct parse_s * parse_struct )
{
	struct parse_s temp_parse_struct;
	int i = 0;

    i = sscanf( in_str, "%lf%s", &temp_parse_struct.value, temp_parse_struct.name );
	if( i == 2 )
	{
		*parse_struct = temp_parse_struct;
        return TRUE;
	}
    else
        return FALSE;
...
static int parse_strtok( char * in_str, struct parse_s * parse_struct )
{
	...
	temp_parse_struct.value = strtod( token, &errStr );
	if( strcmp( errStr, "" ) != 0)
		return FALSE;
	...
	strcpy( temp_parse_struct.name, token );
	if( strlen( temp_parse_struct.name ) > NAME_LEN )
		return FALSE;
